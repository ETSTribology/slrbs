...
// Strategy implementations for built-in scenarios
class MarbleBoxScenario : public ScenarioStrategy {
public:
    virtual void create(RigidBodySystem& rigidBodySystem) override {
        Scenarios::createMarbleBox(rigidBodySystem);
    }
    ...
    virtual std::string getDescription() const override {
        return "Box filled with spherical marbles arranged in a grid pattern";
    }
};

class SphereOnBoxScenario : public ScenarioStrategy {
public:
    virtual void create(RigidBodySystem& rigidBodySystem) override {
        Scenarios::createSphereOnBox(rigidBodySystem);
    }
    ...
    virtual std::string getDescription() const override {
        return "Simple sphere falling on a box with spinning motion";
    }
};

class SwingingBoxesScenario : public ScenarioStrategy {
public:
    virtual void create(RigidBodySystem& rigidBodySystem) override {
        Scenarios::createSwingingBoxes(rigidBodySystem);
    }
    
    virtual std::string getName() const override {
        return "SwingingBoxes";
    }
    
    virtual std::string getDescription() const override {
        return "Chain of boxes connected by hinge joints";
    }
};

class CylinderOnPlaneScenario : public ScenarioStrategy {
public:
    virtual void create(RigidBodySystem& rigidBodySystem) override {
        Scenarios::createCylinderOnPlane(rigidBodySystem);
    }
    
    virtual std::string getName() const override {
        return "CylinderOnPlane";
    }
    
    virtual std::string getDescription() const override {
        return "Cylinder colliding with a ground plane";
    }
};

class CarScenario : public ScenarioStrategy {
public:
    virtual void create(RigidBodySystem& rigidBodySystem) override {
        Scenarios::createCarScene(rigidBodySystem);
    }
    ...
    virtual std::string getDescription() const override {
        return "Simple car with chassis and four wheels connected by hinges";
    }
};

class RopeBridgeScenario : public ScenarioStrategy {
public:
    virtual void create(RigidBodySystem& rigidBodySystem) override {
        Scenarios::createRopeBridgeScene(rigidBodySystem);
    }
    ...
    virtual std::string getDescription() const override {
        return "A rope bridge made of planks connected by joints";
    }
};

// JSON-based scenario strategy
class JsonScenario : public ScenarioStrategy {
public:
    JsonScenario(const std::string& filename) : m_filename(filename) {
        ...
        if (lastDot == std::string::npos || lastDot < lastSlash) {
            m_name = filename.substr(lastSlash);
        } else {
            m_name = filename.substr(lastSlash, lastDot - lastSlash);
        }
    }
    
    virtual void create(RigidBodySystem& rigidBodySystem) override {
        JsonLoader::loadScenarioFromFile(rigidBodySystem, m_filename);
    }
    ...
    virtual std::string getDescription() const override {
        return "Scenario loaded from JSON file: " + m_filename;
    }
...
ScenarioFactory::ScenarioFactory() {
    ...
    Scenarios::registerBuiltInScenarios();
}

void ScenarioFactory::registerScenario(const std::string& name, std::shared_ptr<ScenarioStrategy> strategy) {
    m_scenarios[name] = strategy;
}

bool ScenarioFactory::createScenario(const std::string& name, RigidBodySystem& rigidBodySystem) {
    auto it = m_scenarios.find(name);
    if (it != m_scenarios.end()) {
        it->second->create(rigidBodySystem);
        return true;
    }
    return false;
}

std::vector<std::string> ScenarioFactory::getScenarioNames() const {
    std::vector<std::string> names;
    names.reserve(m_scenarios.size());
    
    for (const auto& pair : m_scenarios) {
...
bool ScenarioFactory::loadFromFile(const std::string& filename, RigidBodySystem& rigidBodySystem) {
    bool success = JsonLoader::loadScenarioFromFile(rigidBodySystem, filename);
    
    if (success) {
        // Register this JSON scenario for future use
        auto scenario = std::make_shared<JsonScenario>(filename);
        registerScenario(scenario->getName(), scenario);
    }
    
    return success;
}

bool ScenarioFactory::saveToFile(const std::string& name, const RigidBodySystem& rigidBodySystem, const std::string& filename) {
    return JsonLoader::saveScenarioToFile(rigidBodySystem, name, filename);
}

// Static scenario implementations
void Scenarios::registerBuiltInScenarios() {
    auto& factory = ScenarioFactory::getInstance();
    
    factory.registerScenario("MarbleBox", std::make_shared<MarbleBoxScenario>());
    factory.registerScenario("SphereOnBox", std::make_shared<SphereOnBoxScenario>());
    factory.registerScenario("SwingingBoxes", std::make_shared<SwingingBoxesScenario>());
    factory.registerScenario("CylinderOnPlane", std::make_shared<CylinderOnPlaneScenario>());
    factory.registerScenario("CarScene", std::make_shared<CarScenario>());
    factory.registerScenario("RopeBridge", std::make_shared<RopeBridgeScenario>());
}
