        ...
        for (const auto& bodyJson : scenario["bodies"]) {
            ...
            if (bodyJson.contains("visual") && body->mesh) {
                ...
                if (visual.contains("color")) {
                    // Store color values directly in RigidBody for later use by SimViewer
                    body->visualProperties["color_r"] = visual["color"]["r"].get<float>();
                    body->visualProperties["color_g"] = visual["color"]["g"].get<float>();
                    body->visualProperties["color_b"] = visual["color"]["b"].get<float>();
                }
                
                // Transparency
                if (visual.contains("transparency")) {
                    body->visualProperties["transparency"] = visual["transparency"].get<float>();
                }
                
                // Smooth shading
                if (visual.contains("smooth_shade")) {
                    body->visualProperties["smooth_shade"] = visual["smooth_shade"].get<bool>();
                }
                
                // Edge width
                if (visual.contains("edge_width")) {
                    body->visualProperties["edge_width"] = visual["edge_width"].get<float>();
                }
            }
