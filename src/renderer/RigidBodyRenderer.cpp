...
RigidBodyRenderer::RigidBodyRenderer() 
    : showContacts(true), showJoints(true), showBoundingBoxes(false) {
}
...
void RigidBodyRenderer::initialize() {
    // Initialize materials
    materialLibrary.initialize();
    
    std::cout << "RigidBodyRenderer initialized" << std::endl;
}

void RigidBodyRenderer::updateAll() {
...
void RigidBodyRenderer::updateBodyTransform(const RigidBody& body) {
    ...
    if (it != bodyMeshes.end() && it->second) {
        ...
        it->second->setTransform(glm::make_mat4x4(tm.data()));
    }
}

void RigidBodyRenderer::createBodyMesh(RigidBody& body) {
...
void RigidBodyRenderer::renderBodies(const std::vector<RigidBody*>& bodies) {
...
void RigidBodyRenderer::renderContacts(const std::vector<Contact*>& contacts) {
    static const char* strContacts = "contacts";
    
    if (!showContacts || contacts.empty()) {
        polyscope::removePointCloud(strContacts);
        return;
    }
    
    // Create matrices for contact points and normals
    Eigen::MatrixXf contactP(contacts.size(), 3);
    Eigen::MatrixXf contactN(contacts.size(), 3);
    
    for (size_t i = 0; i < contacts.size(); ++i) {
        contactP.row(i) = contacts[i]->p;
        contactN.row(i) = contacts[i]->n;
    }
    
    // Convert to polyscope format
    std::vector<glm::vec3> points(contacts.size());
    std::vector<glm::vec3> normals(contacts.size());
...
void RigidBodyRenderer::renderJoints(const std::vector<Joint*>& joints) {
    static const char* strJointPoints = "jointsPoint";
    static const char* strJointCurve = "jointsCurve";
    
    if (!showJoints || joints.empty()) {
    ...
    for (size_t i = 0; i < joints.size(); ++i) {
        ...
        // Add points
        points.emplace_back(p0.x(), p0.y(), p0.z());
...
void RigidBodyRenderer::renderBoundingBoxes(const std::vector<RigidBody*>& bodies) {
    static const char* strBoundingBoxes = "boundingBoxes";
    
    if (!showBoundingBoxes || bodies.empty()) {
...
void RigidBodyRenderer::assignMaterial(RigidBody& body, const std::string& materialName) {
    auto it = bodyMeshes.find(&body);
    if (it != bodyMeshes.end() && it->second) {
